...
class ResetPasswordTests(EventTestMixin, CacheIsolationTestCase):
    ...
    def test_password_rest_fail(self):
        """Tests that if we providing mismatching passwords user is not marked as active"""
        self.assertFalse(self.user.is_active)
        ...
        password_reset_confirm_wrapper(confirm_request, self.uidb36, self.token)
